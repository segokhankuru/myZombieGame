...
namespace Mirror.FizzySteam
{
  public class LegacyServer : LegacyCommon, IServer
  {
    ...
    public static LegacyServer CreateServer(FizzyFacepunch transport, int maxConnections)
    {
      ...
      // BUNKER YAMASI (2026-09-05, ADR-0007): Mirror 96'da OnServerConnected
      // obsolete; adresi de tasiyan surumu kullaniliyor. Adres burada bir IP degil
      // SteamId - Steam tasimasinda "adres" budur.
      s.OnConnected += (id) => transport.OnServerConnectedWithAddress.Invoke(id, s.ServerGetClientAddress(id));
      s.OnDisconnected += (id) => transport.OnServerDisconnected.Invoke(id);
      s.OnReceivedData += (id, data, channel) => transport.OnServerDataReceived.Invoke(id, new ArraySegment<byte>(data), channel);
      // BUNKER YAMASI (2026-09-05, ADR-0007): Mirror 96 hata kanalini IKIYE ayirdi -
      // OnServerError(TransportError, string) beklenen hatalar icin,
      // OnServerTransportException(Exception) beklenmeyenler icin. Fizzy bir
      // Exception uretiyor, yani dogru kanal ikincisi.
      s.OnReceivedError += (id, exception) => transport.OnServerTransportException.Invoke(id, exception);
